...
function createMockHttpAdapter(): MockHttpAdapter {
  ...
  return {
    ...
    mockRequest,
    setResponse: (response: unknown) => { mockResponse = response; mockError = null; sideEffect = null; },
    setError: (error: Error) => { mockError = error; sideEffect = null; },
    setSideEffect: (fn: (url: string, options: Record<string, unknown>) => unknown) => { sideEffect = fn; mockError = null; },
  };
}
...
describe('getAllProfiles', () => {
  ...
  test('throws on HTTP error', async () => {
    ...
    await expect(getAllProfiles(api, 'invalid-key')).rejects.toThrow('401 Unauthorized');
  });
  ...
  test('throws on connection error', async () => {
    ...
    await expect(getAllProfiles(api, 'test-api-key')).rejects.toThrow('Connection refused');
  });
});
...
describe('manageDomain - integration', () => {
  ...
  test('handles partial failure across multiple profiles', async () => {
    ...
    expect(result.failCount).toBe(1);
    expect(result.results.find(r => r.profileId === 'def456')?.success).toBe(false);
  });
});
