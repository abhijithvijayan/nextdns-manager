...
export const syncCommand = new Command('sync')
  ...
  .option('-l, --list <type>', 'Which list to sync: allowlist, denylist, or both', 'both')
  .option('--dry-run', 'Show what would be synced without making changes')
  .option('-p, --profiles <ids...>', 'Specific profile IDs to sync (default: all profiles)')
  .action(async (options) => {
    ...
    if (!validTypes.includes(list)) {
      console.error(`Error: Invalid list type '${list}'. Valid types: ${validTypes.join(', ')}`);
      process.exit(1);
    }
    ...
    try {
      ...
      const result = await syncLists(
        ...
        {
          onProgress: (syncResult: SyncResult, completed: number, total: number) => {
            const op = syncResult.operation;
            const status = syncResult.success ? '' : ` (FAILED: ${syncResult.error})`;
...
function printAnalysis(analysis: SyncAnalysis, listType: SyncTarget): void {
  ...
  console.log(`Total unique allowlist domains: ${analysis.totalUniqueInAllowlist}`);

  let totalOps = 0;
  if (listType === 'both' || listType === 'denylist') {
    totalOps += analysis.denylist.toAdd.length + analysis.denylist.toUpdate.length;
  }
  if (listType === 'both' || listType === 'allowlist') {
    totalOps += analysis.allowlist.toAdd.length + analysis.allowlist.toUpdate.length;
  }
...
function printDryRunPreview(analysis: SyncAnalysis, listType: SyncTarget): void {
  if (listType === 'both' || listType === 'denylist') {
    ...
    if (denyOps.length > 0) {
      ...
      for (const op of denyOps.slice(0, 20)) {
        ...
        const opType = op.type === 'add' ? 'ADD' : 'UPD';
        console.log(`  ${opType} [${op.profileName.slice(0, 15).padEnd(15)}] ${op.domain} (${status})`);
      }
  ...
  if (listType === 'both' || listType === 'allowlist') {
    const allowOps = [...analysis.allowlist.toAdd, ...analysis.allowlist.toUpdate];
    console.log(
